...
fn expand_supply_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(Stp258Standard::reserve(DNAR, &SERPER, 90));
			assert_ok!(Stp258Standard::reserve(JUSD, &SERPER, 100 * 1_000));
			assert_eq!(Stp258Standard::reserved_balance(DNAR, &SERPER), 100);
			assert_eq!(Stp258Standard::reserved_balance(JUSD, &SERPER), 100 * 1_000);
			assert_eq!(Stp258Standard::total_issuance(DNAR), 400);
			assert_eq!(Stp258Standard::total_issuance(JUSD), 400 * 1_000);
			assert_ok!(<Stp258Standard as SerpMarket<AccountId>>::expand_supply(
				DNAR, 
				JUSD, 
				40 * 1_000, 
				2, 
...
fn contract_supply_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(<Stp258Standard as SerpMarket<AccountId>>::contract_supply(
				DNAR, 
				JUSD, 
				40 * 1_000, 
				4, 
